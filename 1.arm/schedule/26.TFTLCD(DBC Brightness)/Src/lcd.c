...
/**
 * @brief       设置PWM占空比来控制背光亮度
 * @param       pwm: PWM占空比 (0-255)
 * @retval      无
 */
void lcd_set_backlight_by_pwm(uint8_t duty)
{
	//读取Manual Brightness Mode (0xD0) A[6]
	lcd_wr_regno(0xD1); // Get DBC Configuration
	uint8_t dbc_conf = lcd_rd_data();
	//printf("dbc_conf: %x\n", dbc_conf);
	if(0 == (dbc_conf & 0x40)){
		lcd_wr_regno(0xD0); // Set DBC Configuration
		//lcd_wr_data(0x68);  // A[6] = 1(关闭DBC); A [5] = 1; A [3：2] = 10(DBC节能正常模式); A [0] = 0(关闭DBC主使能)
		//01101000——0x68
		lcd_wr_data(0x2D);  // A[6] = 0(开启DBC); A [5] = 1; A [3：2] = 11(DBC节能激进模式); A [0] = 1(开启DBC主使能)
		//00101101——0x2D
		
	}

  lcd_wr_regno(0xBE); // Set PWM Configuration  
	lcd_wr_data(0x06); // PWM频率 PWMF[7:0] = 0x06(if (0xD0)A[0] = 1,将忽略这个参数；if (0xD0)A[0] = 0 PWM duty cycle = PWM[7:0]/256)
	lcd_wr_data(duty); // PWM占空比 PWM[7:0] = duty;
  //lcd_wr_data(0x01); // C[3] = 0; C[0] = 1(PWM使能)
	lcd_wr_data(0x09); // C[3] = 1(DBC使能); C[0] = 1(PWM使能)
	lcd_wr_data(0x11); // DBC manual brightness D[7:0]——PWM duty cycle = DBC output * D[7:0]/255
	lcd_wr_data(0x00); // DBC minimum brightness D[7:0]
 ...
 * @retval      当前亮度值 (0-255)
 */
uint8_t lcd_get_backlight_by_pwm(void)
{
    uint8_t duty;
    lcd_wr_regno(0xBF); // Get PWM Configuration
		uint8_t freq = lcd_rd_data();  //PWM频率
    duty = lcd_rd_data(); // 读取当前PWM占空比（亮度）
...
{		
    ...
    if (lcddev.id == 0x7789 || lcddev.id == 0x9341 || lcddev.id == 0x1963)  /* 7789/9341/1963 提速 */
    {
        ...
        //fsmc_write_handle.AddressSetupTime = 3; /* 地址建立时间(ADDSET)为3个fsmc_ker_ck=6*3=18ns */
				...
				FSMC_Bank1E->BWTR[6]|=3<<0;     //地址建立时间(ADDSET)为3个HCLK =24ns
				...
				//FSMC_NORSRAM_Extended_Timing_Init(hsram2.Extended, &fsmc_write_handle, hsram2.Init.NSBank, hsram2.Init.ExtendedMode);
		}
		else if (lcddev.id == 0x5310 || lcddev.id == 0x7796 || lcddev.id == 0x5510 || lcddev.id == 0x9806)  /* 如果是这几个IC, 则设置WR时序为最快 */
		{
				...
				FSMC_Bank1E->BWTR[6]&=~(0XF<<0);//地址建立时间(ADDSET)清零
				FSMC_Bank1E->BWTR[6]|=2<<0;     //地址建立时间(ADDSET)为2个HCLK =12ns
