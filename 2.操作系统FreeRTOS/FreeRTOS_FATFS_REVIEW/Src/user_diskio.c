...
/**
 * 对于25Q128 FLASH芯片, 我们规定�? 12M 给FATFS使用, 12M以后
 * 紧跟字库, 3个字�? + UNIGBK.BIN, 总大�?3.09M, 共占�?15.09M
 * 15.09M以后的存储空间大家可以随便使�?. 
 */

#define SPI_FLASH_SECTOR_SIZE   512
#define SPI_FLASH_SECTOR_COUNT  12 * 1024 * 2   /* 25Q128, �?12M字节给FATFS占用 */
#define SPI_FLASH_BLOCK_SIZE    8               /* 每个BLOCK�?8个扇�? */
#define SPI_FLASH_FATFS_BASE    0               /* FATFS 在外部FLASH的起始地�? �?0�?�? */
/* Private variables ---------------------------------------------------------*/
/* Disk status */
    ...
    switch (cmd) {
		...
		case GET_BLOCK_SIZE:
				*(WORD *)buff = SPI_FLASH_BLOCK_SIZE;
				res = RES_OK;
				break;
		case GET_SECTOR_COUNT:  //块数
				//printf("USER_ioctl->GET_SECTOR_COUNT");
